...
const INTAKE_URL = 'https://shukway-intake.onrender.com/api/coupon-intake'; // live intake → Monday board 5098280692
...
async function flush(){
  ...
  for(const rec of pending){
    ...
    const r=await fetch(INTAKE_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(rec.payload)});
    if(!r.ok) throw new Error('http '+r.status); // throw → Background Sync retries with backoff
    const j=await r.json().catch(()=>({}));
    if(!j || !j.ok) throw new Error('intake rejected');
    rec.status='confirmed'; rec.confirmId=j.itemId||'';
    await idbPut(rec);
